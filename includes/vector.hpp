...
class vector {
	...
	public:
		...
		iterator erase (iterator position) {
			size_type delPos = getPositionElem(position);
			_alloc.destroy(_array + delPos);
			for (size_type i = delPos; i != _size - 1; ++i)
				_array[i] = _array[i + 1];
			_size--;
			return iterator(_array + delPos);
		}
		...
		size_type	getPositionElem(iterator pos) {
			for (size_type i = 0; i < _size; ++i)
				if (_array[i] == *pos)
					return i;
			return 0;
